...
# Exception handler — har qanday handler ichidagi xato log'ga yoziladi
def handle_bot_exception(exc):
    log.exception("Bot handler xatosi: %s", exc)

bot.exception_handler = type('EH', (), {'handle': staticmethod(lambda e: log.exception("Bot xatosi: %s", e))})()
...
def show_main_menu(chat_id, uid=None):
    """Asosiy menyu — faqat ilovani ochish tugmasi.
    Barcha funksiyalar Mini App ichida joylashgan."""
    ...
    if MINI_APP_URL:
        markup.add(types.InlineKeyboardButton(
            "📱  Ilovani ochish", web_app=types.WebAppInfo(url=MINI_APP_URL)))
    markup.add(types.InlineKeyboardButton("📢  Kanalga o'tish", url=KANAL_LINKI))

    bot.send_message(
        chat_id,
        f"👋 Xush kelibsiz, *{name}*!\n\n"
        f"{unvon}  ·  {level}-daraja  ·  💰 {ball} ball\n\n"
        f"Barcha funksiyalar ilova ichida:\n"
        f"📋 Rejalar  ·  🕌 Namoz  ·  🎯 Odatlar\n"
        f"🗺 Orzular  ·  📊 Statistika  ·  🏆 Ball",
        reply_markup=markup,
        parse_mode="Markdown"
    )

# Eski sub-menu funksiyalari — endi ishlatilmaydi, ilovaga yo'naltiriladi
def _redirect_to_app(chat_id):
    markup = types.InlineKeyboardMarkup()
    if MINI_APP_URL:
        markup.add(types.InlineKeyboardButton(
            "📱 Ilovani ochish", web_app=types.WebAppInfo(url=MINI_APP_URL)))
    bot.send_message(chat_id,
        "Bu funksiya ilova ichida mavjud 👆",
        reply_markup=markup)

def show_namoz_menu(chat_id):   _redirect_to_app(chat_id)
def show_kunlik_menu(chat_id):  _redirect_to_app(chat_id)
def show_haftalik_menu(chat_id):_redirect_to_app(chat_id)
def show_hisobot_menu(chat_id): _redirect_to_app(chat_id)
def show_habits_menu(chat_id):  _redirect_to_app(chat_id)
def show_goals_menu(chat_id):   _redirect_to_app(chat_id)
def show_zikr_menu(chat_id):    _redirect_to_app(chat_id)
...
@bot.callback_query_handler(func=lambda c: c.data == "quick_report")
def cb_quick_report(call):
    ...
    try:
        ...
        log.exception("quick_report xatosi")
        bot.send_message(uid, "❌ Hisobot yuklanmadi.")
...
INIT_DATA_MAX_AGE = 48 * 60 * 60  # 48 soat
...
# ─── ORZULAR XARITASI ────────────────────────────────────────────────────────
DREAM_CATEGORIES = ['shaxsiy','oila','ish','soglik','diniy','talim','moliyaviy','sayohat']
DREAM_COLORS     = ['accent','pink','green','amber','purple','cyan','gold','teal']

@api.route('/api/dream/list')
def api_dream_list():
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    conn = request_db()
    rows = conn.execute(
        "SELECT id,title,emoji,category,description,color,status,created_at,achieved_at "
        "FROM dreams WHERE user_id=? ORDER BY status='active' DESC, id DESC", (uid,)).fetchall()
    return jsonify({"dreams": [dict(r) for r in rows]})

@api.route('/api/dream/add', methods=['POST'])
def api_dream_add():
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    body = request.get_json(force=True) or {}
    title = (body.get('title') or '').strip()[:120]
    if not title: return jsonify({"error": "invalid"}), 400
    emoji    = (body.get('emoji') or '⭐')[:8]
    category = body.get('category') or 'shaxsiy'
    if category not in DREAM_CATEGORIES: category = 'shaxsiy'
    desc  = (body.get('description') or '').strip()[:500]
    color = body.get('color') or 'accent'
    if color not in DREAM_COLORS: color = 'accent'
    conn = request_db()
    conn.execute(
        "INSERT INTO dreams (user_id,title,emoji,category,description,color,status,created_at) "
        "VALUES (?,?,?,?,?,?,'active',?)",
        (uid, title, emoji, category, desc, color, today_str()))
    conn.commit()
    return jsonify({"ok": True})

@api.route('/api/dream/achieve', methods=['POST'])
def api_dream_achieve():
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    did = (request.get_json(force=True) or {}).get('id')
    conn = request_db()
    row = conn.execute("SELECT id,status FROM dreams WHERE id=? AND user_id=?", (did, uid)).fetchone()
    if not row: return jsonify({"error": "not_found"}), 404
    if row['status'] == 'active':
        conn.execute("UPDATE dreams SET status='achieved',achieved_at=? WHERE id=?", (today_str(), did))
        add_ball_conn(conn, uid, 100)
    else:
        conn.execute("UPDATE dreams SET status='active',achieved_at=NULL WHERE id=?", (did,))
    conn.commit()
    return jsonify({"ok": True})

@api.route('/api/dream/delete', methods=['POST'])
def api_dream_delete():
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    did = (request.get_json(force=True) or {}).get('id')
    conn = request_db()
    row = conn.execute("SELECT id FROM dreams WHERE id=? AND user_id=?", (did, uid)).fetchone()
    if not row: return jsonify({"error": "not_found"}), 404
    conn.execute("DELETE FROM dreams WHERE id=?", (did,))
    conn.commit()
    return jsonify({"ok": True})
...
@api.route('/api/namoz/times/save', methods=['POST'])
def api_namoz_times_save():
    """Yangi frontend uchun alias — ixtiyoriy maydonlar bilan ishlaydi."""
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    body = request.get_json(force=True) or {}
    names = ['bomdod','peshin','asr','shom','xufton']
    conn = request_db()
    existing = conn.execute("SELECT bomdod,peshin,asr,shom,xufton FROM namoz_times WHERE user_id=?", (uid,)).fetchone()
    vals = {n: (existing[n] if existing else '00:00') for n in names}
    updated = False
    for n in names:
        v = (body.get(n) or '').strip()
        if v:
...
@api.route('/api/namoz/set', methods=['POST'])
def api_namoz_set():
    """Yangi frontend uchun — namoz_nomi + holat (DB formatida)."""
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    body = request.get_json(force=True) or {}
    namoz_nomi = body.get('namoz_nomi', '')
    holat_in   = body.get('holat', '')
    # namoz_nomi ham key (bomdod) ham to'liq nom (Bomdod ☁️) bo'lishi mumkin
    if namoz_nomi in NAMOZ_KEY_MAP:
        namoz_nomi = NAMOZ_KEY_MAP[namoz_nomi]
    valid_nomlar = list(NAMOZ_KEY_MAP.values())
    if namoz_nomi not in valid_nomlar:
        return jsonify({"error": "invalid"}), 400
    holat_db = NAMOZ_HOLAT_TO_DB.get(holat_in) or holat_in
    if holat_db not in ('oqildi','endi_oqiyman','qazo'):
        return jsonify({"error": "invalid holat"}), 400
    conn = request_db()
    ...
    if existing:
        ...
        conn.execute("UPDATE namoz_stats SET holat=? WHERE id=?", (holat_db, existing['id']))
    else:
...
@api.route('/api/weekly-task/add', methods=['POST'])
def api_weekly_task_add():
    """Yangi frontend uchun alias — /api/weekly/add bilan bir xil."""
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    body = request.get_json(force=True) or {}
    name      = (body.get('task_name') or '').strip()
    task_time = (body.get('task_time') or body.get('time') or '').strip()
    if not name: return jsonify({"error": "invalid"}), 400
    conn = request_db()
    conn.execute("INSERT INTO weekly_tasks (user_id,task_name,task_time,category,priority,active) VALUES (?,?,?,?,?,1)",
                 (uid, name, task_time, 'Umumiy', 'oddiy'))
    conn.commit()
    return jsonify({"ok": True})
...
def api_reward_add():
    ...
    try: ball_cost = max(1, int(body.get('ball_cost') or 100))
    except (ValueError, TypeError): ball_cost = 100
    if not title: return jsonify({"error": "invalid"}), 400
    conn = request_db()
    conn.execute("INSERT INTO rewards (user_id,title,ball_cost,created) VALUES (?,?,?,?)",
                 (uid, title, ball_cost, today_str()))
    conn.commit()
    return jsonify({"ok": True})

@api.route('/api/reward/buy', methods=['POST'])
def api_reward_buy():
    uid = get_request_user()
    if uid is None: return jsonify({"error": "unauthorized"}), 401
    rid = (request.get_json(force=True) or {}).get('id')
    conn = request_db()
    reward = conn.execute("SELECT id,ball_cost FROM rewards WHERE id=? AND user_id=?", (rid, uid)).fetchone()
    if not reward: return jsonify({"error": "not_found"}), 404
    urow = conn.execute("SELECT ball FROM users WHERE user_id=?", (uid,)).fetchone()
    if not urow or urow['ball'] < reward['ball_cost']:
        return jsonify({"error": "insufficient_ball"}), 400
    add_ball_conn(conn, uid, -reward['ball_cost'])
    conn.commit()
    return jsonify({"ok": True})
